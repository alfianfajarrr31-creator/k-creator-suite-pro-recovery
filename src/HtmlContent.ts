export const HTML_CONTENT = `
    ...
    <main class="flex-1 flex overflow-hidden relative">
        ...
        <section id="tab-design" class="flex-1 flex overflow-hidden hidden">
            <aside id="designAside" class="mobile-sidebar w-80 border-r border-slate-855 bg-[#08090e] p-4 overflow-y-auto space-y-3 flex flex-col shrink-0">
                <div class="space-y-1">
                    <h2 class="text-xs font-bold text-fuchsia-400 uppercase tracking-wider font-mono">Recruitment Designer — Checkpoint 3</h2>
                    <p class="text-xs text-slate-400 leading-relaxed">Live preview, template lokal, dan export prompt. Seluruh proses tetap cost Rp0.</p>
                </div>
                <div class="rounded-2xl border border-fuchsia-500/20 bg-fuchsia-500/5 p-3 space-y-2">
                    <div class="flex items-center justify-between"><p class="text-[10px] font-black uppercase tracking-wider text-fuchsia-300">Kelengkapan</p><span id="designProgressValue" class="text-xs font-black text-white">0%</span></div>
                    <div class="h-2 rounded-full bg-black/40 overflow-hidden"><div id="designProgressBar" class="h-full w-0 rounded-full bg-fuchsia-500 transition-all duration-300"></div></div>
                    <p id="designRequiredStatus" class="text-[10px] text-slate-400">0/4 data wajib lengkap</p>
                </div>
                <div class="space-y-2">
                    <details class="group rounded-2xl border border-slate-800 bg-[#0b0d14]" open>
                        <summary class="cursor-pointer list-none flex items-center justify-between p-3 text-xs font-black text-white"><span>1. Perusahaan & Brand</span><span class="text-fuchsia-400 group-open:rotate-180 transition">⌄</span></summary>
                        <div class="px-3 pb-3 space-y-3 border-t border-slate-800/60 pt-3">
                            ...
                            <div class="space-y-2"><label class="block text-xs font-semibold text-slate-400">Industri</label><select id="designIndustry" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200"><option value="">Pilih industri</option><option>Technology</option><option>Retail</option><option>Hospitality</option><option>Food & Beverage</option><option>Healthcare</option><option>Education</option><option>Finance</option><option>Logistics</option><option>Manufacturing</option><option>Other</option></select></div>
                            <input id="designLogoInput" type="file" accept="image/png,image/jpeg,image/webp" class="hidden"><input id="designLogoName" type="hidden"><input id="designPrimaryColor" type="hidden"><input id="designSecondaryColor" type="hidden">
                            <button data-action="upload-design-logo" class="w-full py-2.5 rounded-xl border border-dashed border-fuchsia-500/40 bg-fuchsia-500/5 text-xs font-bold text-fuchsia-200 cursor-pointer">Upload & Analisis Logo</button>
                            <div id="designLogoEmpty" class="text-[10px] text-slate-500 text-center">Belum ada logo.</div>
                            <div id="designLogoPreviewWrap" class="hidden rounded-xl border border-slate-800 bg-black/30 p-2 space-y-2"><div class="flex items-center gap-3"><div class="w-14 h-14 rounded-lg bg-white/90 p-1 flex items-center justify-center"><img id="designLogoPreview" class="max-w-full max-h-full object-contain"></div><div class="min-w-0 flex-1"><p id="designLogoFileName" class="text-xs font-bold text-white truncate"></p><button data-action="remove-design-logo" class="mt-1 text-[10px] font-bold text-rose-300 cursor-pointer">Hapus logo</button></div></div><div class="grid grid-cols-2 gap-2"><div class="rounded-lg bg-[#0f111a] p-2"><div class="flex items-center gap-2"><span id="designPrimarySwatch" class="w-4 h-4 rounded-full bg-slate-700 border border-white/20"></span><span class="text-[9px] text-slate-500">Primary</span></div><p id="designPrimaryColorLabel" class="mt-1 text-[10px] font-mono text-slate-300">Belum dianalisis</p></div><div class="rounded-lg bg-[#0f111a] p-2"><div class="flex items-center gap-2"><span id="designSecondarySwatch" class="w-4 h-4 rounded-full bg-slate-500 border border-white/20"></span><span class="text-[9px] text-slate-500">Secondary</span></div><p id="designSecondaryColorLabel" class="mt-1 text-[10px] font-mono text-slate-300">Belum dianalisis</p></div></div></div>
                        </div>
                    </details>
                    <details class="group rounded-2xl border border-slate-800 bg-[#0b0d14]" open>
                        <summary class="cursor-pointer list-none flex items-center justify-between p-3 text-xs font-black text-white"><span>2. Posisi & Penempatan</span><span class="text-fuchsia-400 group-open:rotate-180 transition">⌄</span></summary>
                        <div class="px-3 pb-3 space-y-3 border-t border-slate-800/60 pt-3">
                            <div class="space-y-2"><label class="block text-xs font-semibold text-slate-400">Posisi Lowongan *</label><input id="designPosition" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="HR Recruitment Specialist"></div>
                            <div class="grid grid-cols-2 gap-2"><div><label class="block text-xs font-semibold text-slate-400 mb-2">Tipe Kerja</label><select id="designEmploymentType" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200"><option>Full Time</option><option>Contract</option><option>Part Time</option><option>Internship</option><option>Freelance</option></select></div><div><label class="block text-xs font-semibold text-slate-400 mb-2">Mode Kerja</label><select id="designWorkMode" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200"><option>WFO</option><option>Hybrid</option><option>Remote / WFH</option></select></div></div>
                            <input id="designLocation" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="Lokasi penempatan"><input id="designSalary" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="Informasi gaji (opsional)">
                        </div>
                    </details>
                    <details class="group rounded-2xl border border-slate-800 bg-[#0b0d14]"><summary class="cursor-pointer list-none flex items-center justify-between p-3 text-xs font-black text-white"><span>3. Requirement & Jobdesk</span><span class="text-fuchsia-400 group-open:rotate-180 transition">⌄</span></summary><div class="px-3 pb-3 space-y-3 border-t border-slate-800/60 pt-3"><textarea id="designRequirements" class="w-full h-28 resize-y rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="Requirement * (satu per baris)"></textarea><textarea id="designResponsibilities" class="w-full h-24 resize-y rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="Jobdesk (satu per baris)"></textarea></div></details>
                    <details class="group rounded-2xl border border-slate-800 bg-[#0b0d14]"><summary class="cursor-pointer list-none flex items-center justify-between p-3 text-xs font-black text-white"><span>4. Benefit & Lamaran</span><span class="text-fuchsia-400 group-open:rotate-180 transition">⌄</span></summary><div class="px-3 pb-3 space-y-3 border-t border-slate-800/60 pt-3"><textarea id="designBenefits" class="w-full h-24 resize-y rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="Benefit (satu per baris)"></textarea><input id="designDeadline" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="Deadline"><input id="designContact" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="Email / WhatsApp / URL *"><select id="designCTA" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200"><option>Apply Now</option><option>Join Our Team</option><option>Send Your CV</option><option>Apply Today</option><option>We're Hiring</option></select></div></details>
                    <details class="group rounded-2xl border border-slate-800 bg-[#0b0d14]"><summary class="cursor-pointer list-none flex items-center justify-between p-3 text-xs font-black text-white"><span>5. Style & Output</span><span class="text-fuchsia-400 group-open:rotate-180 transition">⌄</span></summary><div class="px-3 pb-3 space-y-3 border-t border-slate-800/60 pt-3"><select id="designStyle" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200"><option>Modern Corporate</option><option>Minimalist Professional</option><option>Luxury Corporate</option><option>Bold Recruitment</option><option>Friendly Startup</option><option>Creative Agency</option><option>Technology</option><option>Healthcare</option><option>Education</option></select><select id="designRatio" class="w-full rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200"><option value="4:5">Instagram Feed — 4:5</option><option value="1:1">Square — 1:1</option><option value="9:16">Story / Reels — 9:16</option><option value="16:9">LinkedIn Landscape — 16:9</option><option value="A4">A4 Portrait</option><option value="A3">A3 Portrait</option></select><textarea id="designExtraNotes" class="w-full h-20 rounded-xl bg-[#0f111a] border border-slate-800 p-2.5 text-xs text-slate-200" placeholder="Catatan visual tambahan"></textarea></div></details>
                </div>
                <div class="grid grid-cols-2 gap-2"><button data-action="generate-design-recruitment" class="col-span-2 py-3 rounded-xl bg-fuchsia-600 hover:bg-fuchsia-500 text-white text-xs font-black cursor-pointer">Generate Prompt</button><button data-action="save-design-draft" class="py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs font-bold text-slate-300 cursor-pointer">Simpan Draft</button><button data-action="load-design-draft" class="py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs font-bold text-slate-300 cursor-pointer">Muat Draft</button><button data-action="save-design-template" class="py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/25 text-xs font-bold text-emerald-300 cursor-pointer">Simpan Template</button><button data-action="load-design-template" class="py-2 rounded-xl bg-sky-500/10 border border-sky-500/25 text-xs font-bold text-sky-300 cursor-pointer">Muat Template</button><button data-action="clear-design-draft" class="col-span-2 py-2 rounded-xl bg-rose-500/10 border border-rose-500/20 text-xs font-bold text-rose-300 cursor-pointer">Reset Form</button></div>
            </aside>
            <section class="flex-1 overflow-y-auto bg-[#05060a] p-4 md:p-7">
                <div class="max-w-7xl mx-auto space-y-5">
                    <div class="rounded-3xl border border-fuchsia-500/20 bg-gradient-to-br from-fuchsia-500/10 to-transparent p-5 md:p-6"><p class="text-[10px] font-black text-fuchsia-300 uppercase tracking-[0.22em]">Design Studio — Zero Cost</p><h1 class="mt-2 text-2xl md:text-3xl font-black text-white">Recruitment Designer</h1><p class="mt-2 text-sm text-slate-400">Isi form di kiri. Poster di bawah berubah secara langsung tanpa AI dan tanpa biaya.</p></div>
                    <div class="grid grid-cols-1 xl:grid-cols-[minmax(340px,0.9fr)_minmax(420px,1.1fr)] gap-5">
                        <div class="space-y-4">
                            <div class="rounded-3xl border border-slate-800 bg-[#08090e] p-4"><div class="flex items-center justify-between mb-3"><div><p class="text-[10px] font-black text-amber-300 uppercase tracking-wider">Design Recommendation</p><h3 class="text-sm font-black text-white mt-1">Rule-based, tanpa AI</h3></div><span id="designRecommendationSwatch" class="w-9 h-9 rounded-xl border border-white/20 bg-fuchsia-600"></span></div><div class="grid grid-cols-2 gap-2 text-[10px]"><div class="rounded-xl bg-black/30 p-3"><span class="text-slate-500">Style</span><p id="designRecommendationStyle" class="mt-1 font-bold text-slate-200">Modern Corporate</p></div><div class="rounded-xl bg-black/30 p-3"><span class="text-slate-500">Color</span><p id="designRecommendationColor" class="mt-1 font-mono font-bold text-slate-200">#C026D3</p></div><div class="col-span-2 rounded-xl bg-black/30 p-3"><span class="text-slate-500">Hero Visual</span><p id="designRecommendationVisual" class="mt-1 text-slate-300">professional employee or workplace visual</p></div><div class="col-span-2 rounded-xl bg-black/30 p-3"><span class="text-slate-500">Benefit Hint</span><p id="designRecommendationBenefit" class="mt-1 text-slate-300">Tambahkan benefit paling menarik</p></div></div><button data-action="apply-design-recommendation" class="w-full mt-3 py-2 rounded-xl border border-amber-500/30 bg-amber-500/10 text-[10px] font-black text-amber-200 cursor-pointer">Terapkan Saran</button></div>
                            <div class="rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-4 space-y-3"><div class="flex items-center justify-between gap-3"><div><p class="text-[10px] font-black uppercase tracking-wider text-cyan-300">Content Intelligence</p><h3 id="designDensityStatus" class="text-sm font-black text-white mt-1">Kepadatan konten aman</h3></div><span id="designDensityBadge" class="px-2 py-1 rounded-full text-[9px] font-black bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">Siap dipakai</span></div><div class="flex items-center gap-3"><div class="flex-1 h-2 rounded-full bg-slate-900 overflow-hidden"><div id="designContentScoreBar" class="h-full bg-cyan-400 transition-all" style="width:0%"></div></div><strong id="designContentScore" class="text-sm text-cyan-300">0%</strong></div><p id="designDensityDetail" class="text-[10px] text-slate-400">0 poin</p><div class="grid grid-cols-1 sm:grid-cols-2 gap-2"><button data-action="optimize-design-local" class="py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-black text-white">Ringkas Otomatis — Gratis</button><button data-action="optimize-design-ai" class="py-2.5 rounded-xl bg-gradient-to-r from-cyan-600 to-fuchsia-600 text-xs font-black text-white">✨ Ringkas dengan AI</button></div><p class="text-[9px] text-slate-500">Teks asli tetap berada di form sampai tombol optimasi ditekan. AI dilarang menambah syarat atau fakta baru.</p></div><div id="designOutputEmpty" class="rounded-3xl border border-dashed border-slate-800 bg-[#08090e] p-8 text-center"><div class="text-4xl mb-3">📝</div><h3 class="text-sm font-bold text-slate-300">Prompt belum dibuat.</h3><p class="text-xs text-slate-500 mt-1">Live poster tetap bisa dilihat sambil mengisi form.</p></div>
                            <div id="designOutputPanel" class="hidden rounded-3xl border border-fuchsia-500/20 bg-[#08090e] p-5"><div class="flex flex-wrap items-center justify-between gap-2 mb-4"><div><p class="text-[10px] font-black text-fuchsia-300 uppercase tracking-wider">Prompt Output</p><h3 class="text-sm font-black text-white">Siap Copy / Export</h3></div><div class="flex flex-wrap gap-2"><button data-action="copy-design-prompt" class="px-3 py-2 rounded-xl bg-fuchsia-600 text-white text-xs font-black cursor-pointer">Copy</button><button data-action="export-design-txt" class="px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs font-black text-slate-200 cursor-pointer">TXT</button><button data-action="export-design-md" class="px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs font-black text-slate-200 cursor-pointer">Markdown</button></div></div><pre id="designPromptOutput" class="max-h-[480px] overflow-y-auto whitespace-pre-wrap rounded-2xl bg-black/40 border border-slate-800 p-4 text-xs leading-relaxed text-slate-300 font-mono select-text"></pre></div>
                        </div>
                        <div class="rounded-3xl border border-slate-800 bg-[#08090e] p-4 md:p-5"><div class="flex items-center justify-between gap-3 mb-4"><div><p class="text-[10px] font-black text-emerald-300 uppercase tracking-wider">Live Poster Preview</p><h3 class="text-sm font-black text-white mt-1">Berubah otomatis saat form diisi</h3></div><span id="designPreviewRatioLabel" class="px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-[10px] font-mono text-slate-300">4:5</span></div>
                            <div class="flex justify-center overflow-auto rounded-2xl bg-black/30 border border-slate-800 p-4 min-h-[610px]">
                                <article id="designPosterPreview" class="relative overflow-hidden shadow-2xl flex flex-col w-[410px] min-h-[545px] rounded-[26px] bg-white text-slate-950 transition-all duration-300" style="--brand:#C026D3; --brand2:#F5D0FE;">
                                    <div id="designPosterAccent" class="h-3 w-full" style="background:var(--brand)"></div>
                                    <div class="absolute -right-20 -top-16 w-56 h-56 rounded-full opacity-20" style="background:var(--brand)"></div>
                                    <div class="relative z-10 p-7 flex-1 flex flex-col">
                                        <div class="flex items-center justify-between gap-3"><div id="designPosterLogoBox" class="h-12 min-w-12 max-w-[150px] rounded-xl bg-white border border-slate-200 px-2 flex items-center justify-center"><img id="designPosterLogo" class="hidden max-h-9 max-w-[130px] object-contain"><span id="designPosterLogoFallback" class="text-[10px] font-black text-slate-400">YOUR LOGO</span></div><span id="designPosterEmployment" class="px-3 py-1 rounded-full text-[10px] font-black text-white" style="background:var(--brand)">FULL TIME • WFO</span></div>
                                        <div class="mt-8"><p class="text-xs font-black tracking-[0.24em] uppercase" style="color:var(--brand)">We Are Hiring</p><h2 id="designPosterPosition" class="mt-2 text-3xl leading-tight font-black">Your Next Great Role</h2><p id="designPosterCompany" class="mt-2 text-sm font-bold text-slate-500">Nama Perusahaan</p></div>
                                        <div class="mt-5 flex flex-wrap gap-2 text-[10px] font-bold"><span id="designPosterLocation" class="px-3 py-1.5 rounded-full bg-slate-100">📍 Lokasi</span><span id="designPosterSalary" class="px-3 py-1.5 rounded-full bg-slate-100 hidden">💰 Gaji</span></div>
                                        <div class="mt-6 grid grid-cols-1 gap-4"><section><h3 class="text-xs font-black uppercase tracking-wider" style="color:var(--brand)">Requirements</h3><ul id="designPosterRequirements" class="mt-2 space-y-1.5 text-[11px] leading-relaxed text-slate-700"><li>• Tambahkan requirement utama</li></ul></section><section id="designPosterResponsibilitySection" class="hidden"><h3 class="text-xs font-black uppercase tracking-wider" style="color:var(--brand)">Jobdesk</h3><ul id="designPosterResponsibilities" class="mt-2 space-y-1.5 text-[11px] leading-relaxed text-slate-700"><li>• Tambahkan jobdesk utama</li></ul></section><section id="designPosterBenefitSection"><h3 class="text-xs font-black uppercase tracking-wider" style="color:var(--brand)">Benefits</h3><ul id="designPosterBenefits" class="mt-2 space-y-1.5 text-[11px] leading-relaxed text-slate-700"><li>• Tambahkan benefit menarik</li></ul></section></div>
                                        <div class="mt-auto pt-7"><div class="rounded-2xl p-4 text-white" style="background:var(--brand)"><div class="flex items-center justify-between gap-3"><div><p id="designPosterCTA" class="text-sm font-black">Apply Now</p><p id="designPosterContact" class="text-[10px] mt-1 opacity-85 break-all">email@company.com</p></div><div id="designPosterDeadline" class="text-right text-[9px] font-bold opacity-90"></div></div></div></div>
                                    </div>
                                </article>
                            </div>
                        </div>
